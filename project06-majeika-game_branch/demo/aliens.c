...
const double ALIEN_VELOCITY = 100.0;
...
const double NUKE_SPEED = 1000.0;
const double TIME_MIN = .85;
...
body_t *random_alien(scene_t *scene){
  ...
  for (size_t i = 0; i < scene_bodies(scene); i++){
    if (*(int *) body_get_info(scene_get_body(scene, i)) == ENEMY) {
      num_aliens += 1;
    }
  }
  ...
  if (*(int *)body_get_info(body) == FRIENDLY){
    for (size_t i = 1; i < scene_bodies(scene); i++){
      ...
      if (*(int *) body_get_info(curr) == ENEMY){
        create_destructive_collision(scene, body, curr);
      }
    }
  }
  if (*(int *) body_get_info(body) == ALIEN_NUKE){
    body_t *fort = scene_get_body(scene, 0);
    create_destructive_collision(scene, body, fort);
  }
}
...
int main(){
  ...
  collision(scene, fort);
  setup_aliens(scene);
  SDL_Texture *background = loadTexture("images/train.jpg");
