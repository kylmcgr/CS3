...
void compute_new_position(list_t *object_pos, double dt, vector_t *velocity) {
  ...
  for (int i = 0; i < star_points * 2; i++){
    vector_t *curr = list_get(object_pos, i);
...
int main (int argc, char *argv[]) {
  ...
  list_t *object_pos = list_init(10, free);
  double dt = 0.0;
  for (int i = 0; i < 10; i++) {
